...
import "../styles/storefront-mode-style.css";


import "../styles/storefront-template-engine.css";
import { resolveStorefrontTemplate } from "../storefront/templates";
import { SafeStorefrontTemplateRenderer } from "../storefront/StorefrontTemplateRenderer";
const STOREFRONT_MODE_OPTIONS = {
  catalog: {
    key: "catalog",
...
const STOREFRONT_STYLE_OPTIONS = {
  classic: {
    key: "classic",
    label: "Classic",
    accent: "#6B4F2A",
...
export default function Storefront({ tenantSlug = null }) {
  ...
  const storefrontThemeMode = storefrontThemeShop?.storefront_mode || "catalog";
  const storefrontThemeStyle = storefrontThemeShop?.storefront_style || "classic";
  const storefrontModeMeta =
    STOREFRONT_MODE_OPTIONS[storefrontThemeMode] || STOREFRONT_MODE_OPTIONS.catalog;
  const storefrontStyleMeta =
    STOREFRONT_STYLE_OPTIONS[storefrontThemeStyle] || STOREFRONT_STYLE_OPTIONS.classic;
  const activeStorefrontTemplate = resolveStorefrontTemplate(storefrontModeMeta.key, storefrontStyleMeta.key);

  useEffect(() => {
    const root = document.documentElement;

    root.setAttribute("data-lapakin-storefront-mode", storefrontModeMeta.key);
    root.setAttribute("data-lapakin-storefront-style", storefrontStyleMeta.key);
    root.setAttribute("data-lapakin-storefront-template", activeStorefrontTemplate.templateKey);
    root.setAttribute("data-lapakin-storefront-hero", activeStorefrontTemplate.hero);
    root.setAttribute("data-lapakin-storefront-card", activeStorefrontTemplate.productCard);
    root.setAttribute("data-lapakin-storefront-cta", activeStorefrontTemplate.ctaStyle);
    root.setAttribute("data-lapakin-storefront-density", activeStorefrontTemplate.density);
    root.style.setProperty("--lapakin-storefront-accent", storefrontStyleMeta.accent);
    root.style.setProperty("--lapakin-template-accent", activeStorefrontTemplate.tokens.accent);
    root.style.setProperty("--lapakin-template-bg", activeStorefrontTemplate.tokens.background);
    root.style.setProperty("--lapakin-template-surface", activeStorefrontTemplate.tokens.surface);
    root.style.setProperty("--lapakin-template-ink", activeStorefrontTemplate.tokens.ink);
    root.style.setProperty("--lapakin-template-muted", activeStorefrontTemplate.tokens.muted);

    const templateDebugEnabled =
      typeof window !== "undefined" &&
      new URLSearchParams(window.location.search).get("debugTemplate") === "1";

    if (templateDebugEnabled) {
      let badge = document.getElementById("lapakin-storefront-mode-style-badge");

      if (!badge) {
        badge = document.createElement("div");
        badge.id = "lapakin-storefront-mode-style-badge";
        document.body.appendChild(badge);
      }

      badge.textContent = `${activeStorefrontTemplate.label} · ${activeStorefrontTemplate.mood}`;
    } else {
      const currentBadge = document.getElementById("lapakin-storefront-mode-style-badge");
      if (currentBadge) currentBadge.remove();
    }

    return () => {
      root.removeAttribute("data-lapakin-storefront-mode");
      root.removeAttribute("data-lapakin-storefront-style");
      root.removeAttribute("data-lapakin-storefront-template");
      root.removeAttribute("data-lapakin-storefront-hero");
      root.removeAttribute("data-lapakin-storefront-card");
      root.removeAttribute("data-lapakin-storefront-cta");
      root.removeAttribute("data-lapakin-storefront-density");
      root.style.removeProperty("--lapakin-storefront-accent");
      root.style.removeProperty("--lapakin-template-accent");
      root.style.removeProperty("--lapakin-template-bg");
      root.style.removeProperty("--lapakin-template-surface");
      root.style.removeProperty("--lapakin-template-ink");
      root.style.removeProperty("--lapakin-template-muted");

      const currentBadge = document.getElementById("lapakin-storefront-mode-style-badge");
      if (currentBadge) currentBadge.remove();
    };
  }, [
    storefrontModeMeta.key,
    storefrontModeMeta.label,
    storefrontStyleMeta.key,
    storefrontStyleMeta.label,
    storefrontStyleMeta.accent,
  ]);
  ...
  const storefrontRendererSetting =
    shop?.storefront_renderer ||
    data?.shop?.storefront_renderer ||
    data?.store?.storefront_renderer ||
    data?.tenant?.storefront_renderer ||
    data?.data?.shop?.storefront_renderer ||
    data?.data?.store?.storefront_renderer ||
    "legacy";

  const templateRendererPreviewEnabled =
    typeof window !== "undefined" &&
    (
      new URLSearchParams(window.location.search).get("renderer") === "1" ||
      storefrontRendererSetting === "template"
    );

  if (templateRendererPreviewEnabled && data && activeStorefrontTemplate) {
    const templateRendererData = { ...(data || {}) };

    if (typeof shop !== "undefined") {
      templateRendererData.shop = shop;
    }

    if (typeof products !== "undefined") {
      templateRendererData.products = products;
    }

    return (
      <SafeStorefrontTemplateRenderer
        data={templateRendererData}
        template={activeStorefrontTemplate}
      />
    );
  }
