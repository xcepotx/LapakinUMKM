...
async def bulk_update_daily_menu(request: Request):
    """Bulk update `available_days` for many products in one shot.
    Tier-gated (Pro/Bisnis).

    NOTE: This route MUST be registered BEFORE `/products/{product_id}` so FastAPI
    matches the literal path before the path-parameter route.

    Body: {
      "updates": [
        {"product_id": "prod_xxx", "available_days": [0, 1, 2]},
        ...
      ]
    }

    Empty list = setiap hari. Replaces existing available_days entirely.
    """
    user = await require_user(request)
    if not user.get("shop_id"):
    ...
    for u in sane_updates:
        r = await db.products.update_one(
            ...
            {"$set": {"available_days": u["available_days"]}},
        )
