const API_URL = "/api/public/storefront/kopi-senja?compact=1";
const WA_FALLBACK = "081234567890";
...
function orderText() {
  ...
  if (template) {
    return fillTemplate(template, {
      shop_name: shop.name || "Kopi Senja",
      items: lines.join("\n"),
  ...
  return [
    `Halo ${shop.name || "Kopi Senja"}, saya mau pesan:`,
    "",
    lines.join("\n"),
    "",
...
function productText(product) {
  ...
  return `Halo ${shop.name || "Kopi Senja"}, saya mau tanya ${product.name} (${money.format(product.price || 0)}). Apakah masih tersedia?`;
}
...
function updateSeo(shop) {
  const seo = shop.seo || {};
  document.title = seo.title || `${shop.name || "Kopi Senja"} - Slow Coffee Bar`;
  const meta = document.querySelector('meta[name="description"]');
  if (meta) meta.setAttribute("content", seo.description || shop.description || shop.tagline || "Slow coffee bar.");
}

function renderHero() {
  const shop = store.shop || {};
  updateSeo(shop);
  el("heroTitle").textContent = shop.storefront_hero_title || "Kopi pelan untuk sore yang lebih hangat.";
  el("heroCopy").textContent = shop.storefront_hero_subtitle || shop.tagline || shop.description || "Espresso, latte, cold brew, dan pastry untuk jeda yang layak.";
  el("heroCta").textContent = shop.storefront_cta_label || "Lihat menu";
  el("storyText").textContent = shop.about || shop.description || "Kopi diseduh harian dari biji pilihan dan susu segar.";
  ...
  const wa = `https://wa.me/${waNumber(shop.whatsapp)}?text=${encodeURIComponent(`Halo ${shop.name || "Kopi Senja"}, saya mau tanya menu hari ini.`)}`;
...
function renderMenu() {
  ...
  el("menuGrid").innerHTML = shown.map((product) => {
    ...
    const desc = safeText(product.description, "Racikan favorit bar Kopi Senja, siap dibuat saat dipesan.");
...
async function init() {
  try {
    ...
    el("loadingState").innerHTML = `<p style="grid-column:1/-1;color:#7a4a3a;font-weight:800">Menu belum bisa dimuat. Silakan chat Kopi Senja lewat WhatsApp.</p>`;
  }
}
